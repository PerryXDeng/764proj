...
class Mixer:
    def __init__(self, resolution=64, agent=None, config=None, listIn=None, dataPath=None, templatePath=None,
                 ...
                 useTemplate=False, thresholdNN=-1, pcaDataTrainNum=None, pcaComp=2,usePCA=False,applyRandomTest=None,noAdjust=None):
    ...
    def findListtUsingL2(self, partIdx, partType, chairID=None, sendTemplate=False):
        ...
        for i in range(self.listLength):
            ...
            for j in range(self.allChairs[i].nParts):
                ...
                if partChair.type == partType:
                    ...
                    penalty.append(0)
                    if partType == 3:
                        penalty[len(penalty) - 1] = penalty[len(penalty) - 1] + 2.0 * abs(
                            self.allChairs[i].numLegs - numLegs)
                        penalty[len(penalty) - 1] = penalty[len(penalty) - 1] + 2.0 * abs(partChair.legNum -
                                                                                           partTemplate.legNum)
                        if self.legModel != -1:
                            if self.legModel != i:
                                penalty[len(penalty) - 1] = penalty[len(penalty) - 1] + 3
    ...
    def mixNmatchImproved(self, numModels=1, outDir="results/mix", list=None):
        ...
        for modNum in range(numModels):
            ...
            chairListUse[0] = [backListChair[0]]
            partListUse[0] = [backListPart[0]]
            ...
            for genI in range(numParts):
                ...
                partType = self.allChairs[idChairs[0]].partList[idParts[0]].type
    ...
    def outputTemplate(self, dir):
        ...
        for i in range(numParts):
            ...
            partType = self.templateChair.partList[i].type
            if partType != PARTTYPE:
                continue;
            voxels = voxels.cpu().detach().numpy()
            voxels = voxels[0]
